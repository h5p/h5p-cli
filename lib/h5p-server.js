...
function H5PServer(configFile) {
  ...
  this.imageDir = path.join(this.cwd, this.config.images);
...
H5PServer.prototype.start = function (done) {
  ...
  app.use('/images', express.static(path.join(this.cwd, this.config.images)));
...
H5PServer.prototype.getJsonContent = function (contentName) {
  ...
  const contentFile = path.join(this.contentDir, contentName);
...
H5PServer.prototype.getDevConfig = function (currentContent) {
  ...
  var contentFiles = fs.readdirSync(this.contentDir).filter(function (file) {
    return path.extname(file) === '.json';
  });

  var devConfig = {
    contents: contentFiles,
    currentContent: currentContent || contentFiles[0],
...
H5PServer.prototype.createH5PIntegration = function (content, contentName) {
  return extend(true, {
    ...
    "url": "http://" + this.config.host + ':' + this.config.port + '/images',
    "contents": {
      "cid-1": {
