...
var env = {
  'SSH_AUTH_SOCK': process.env['SSH_AUTH_SOCK'], // TODO: Try running ssh-agent if not set?
  'USER': process.env['USER'],
  'GIT_SSH': __dirname + '/../bin/h5p-ssh'
};
...
function skipWarnings(output) {
  if (output.substr(0, 8) === 'Warning:') output = '';
  return output;
}
...
h5p.pack = function (libraries, next) {
  ...
  try {
    for (var i = 0; i < libraries.length; i++) {
      var library = JSON.parse(fs.readFileSync(libraries[i] + '/library.json'));
      archiveDir(archive, libraries[i], library.machineName);
    } 
  }
  catch (err) {
    return next(err);
