...
function getRegistryPromise() {
  if (registry) {
    return Promise.resolve(registry.libraries);
  }
...
function recursivelyAddToCollection(libraryName) {
  if (collection[libraryName]) {
    return;
  }
  collection[libraryName] = registry.libraries[libraryName];

  if (!registry.libraries[libraryName].dependencies){
    return;
  }

  registry.libraries[libraryName].dependencies
    .filter((depName) => !collection[depName])
    .forEach((depName) => {
      recursivelyAddToCollection(depName);
    });
}
...
h5p.recursivePack = function (libraries) {
  collection = collection || {};

  // Verify libraries against registry and get dependencies recursively
  return getRegistryPromise()
    .then((registeredLibraries) => {
      libraries
        .filter(libraryName => !!registeredLibraries[libraryName])
        .forEach((libraryName) => {
          recursivelyAddToCollection(libraryName);
        });
