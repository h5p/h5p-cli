...
/**
 * Will populate existing language files with the original texts.
 *
 *
 */
h5p.addOriginalTexts = function (languageCode, repos, next) {
  processRepos(repos, function (repo, done) {
    ...
    // Go through semantics to find all original text values
    readFile(repo + '/semantics.json', function (err, semantics) {
      if (err) {
        status.failed = true;
        status.msg = err;
        return done(status);
      ...
      // TODO: try and catch
      semantics = JSON.parse(semantics.toString());

      // Get translation
      readFile(repo + '/language/' + languageCode + '.json', function (err, translation) {
        if (err) {
          status.failed = true;
          status.msg = err;
          return done(status);
        ...
        translation = JSON.parse(translation.toString());
        translation.semantics = updateTranslationFields(semantics, translation.semantics);

        fs.writeFileSync(repo + '/language/' + languageCode + '.json', JSON.stringify(translation, null, 2));
 ...
 * @returns {Array}
 */
var updateTranslationFields = function (semantics, translation) {
  var updated = false;
  if (translation === undefined) {
    translation = [];
  }
  for (var i = 0; i < semantics.length; i++) {
    translation[i] = updateTranslationField(semantics[i], translation[i]);
 ...
 * @returns {object}
 */
var updateTranslationField = function (field, oldTranslation) {
  var updatedTranslation = {};
  var updated = false;
  for (var attr in field) {
    if (attr === 'field') {
      // Process single sub-field
      updatedTranslation.field = updateTranslationField(field.field, oldTranslation !== undefined ? oldTranslation.field : undefined);
      ...
      updatedTranslation[attr] = updateTranslationFields(field[attr], oldTranslation !== undefined ? oldTranslation[attr] : undefined);
      if (updatedTranslation[attr] !== undefined) {
        updated = true;
      }
    }
    else if (['label', 'description', 'default', 'entity'].indexOf(attr) !== -1) {
      if (attr === 'default' && field.type !== 'text') {
        continue; // Only translate default for text values
      }

      // Prefix all text string properties
      updated = true;
      updatedTranslation['english' + attr[0].toUpperCase() + attr.substr(1)] = field[attr];
      updatedTranslation[attr] = (oldTranslation !== undefined && oldTranslation[attr] !== undefined ? oldTranslation[attr] : 'TODO');
    }
  }
