...
var Input = require('./utility/input');
 ...
 * @deprecated Use getRegistryPromise instead
 */
...
/**
 * Get officially registered H5Ps from h5p.org
 * @return {Promise}
 */
function getRegistryPromise() {
  return new Promise((resolve, reject) => {
    if (registry) {
      resolve(registry.libraries);
      return;
    }

    https.get('https://h5p.org/registry.json', function (response) {
      if (response.statusCode !== 200) {
        reject('Server responded with HTTP ' + response.statusCode + '.');
      }
      ...
      response.on('end', function () {
        try {
          ...
          reject('Cannot parse registry information: ' + error.message);
        }
      ...
      reject('Cannot connect to server: ' + error.message);
    });
  });
}
 ...
 * @deprecated Use getLibraryPromise instead
 */
...
/**
 * Adds library dependencies to collection.
 *
 * @param {Object} library Library with dependencies
 * @return {Promise} Resolves when all dependencies was found
 */
function addDependenciesToCollection(library) {
  return new Promise((resolve, reject) => {
    if (!library.dependencies) {
      resolve();
    }
    ...
    for (var i = 0; i < library.dependencies.length; i++) {
      let dependency = library.dependencies[i];
      const depPromise = getLibraryPromise(dependency).catch((err) => {
        reject('\n' + dependency + ': ' + err);
      });
    ...
    Promise.all(dependenciesPromises)
      .then(
        () => resolve(),
        (errors) => {
          let errorString = errors.reduce((prev, curr) => prev + curr);
          reject(errorString);
        }
      );
  });
 ...
 * @return {Promise}
 */
function getLibraryPromise(libraryName) {
  return new Promise((resolve, reject) => {
    if (libraryName === '') {
      reject('No library name');
      return;
    }
    getRegistryPromise().then((libraries) => {
      collection = collection || {};
      if (collection[libraryName]) {
        resolve(); // Already have this.
        return;
      }

      let library = libraries[libraryName];
      if (!library) {
        reject('No such library.');
        return;
      }

      collection[libraryName] = library;
      addDependenciesToCollection(library)
        .then(
          () => resolve(),
          (err) => reject(err)
        );
 ...
 * @deprecated Use findRepositoriesPromise instead
 */
 ...
 * @return {Promise}
 */
function findRepositoriesPromise(skipCheck) {
  return new Promise((resolve) => {
    fs.readdir('.', function (error, files) {
      if (error) {
        outputWriter.printError(error);
        reject(error);
        return;
      }
 ...
 * @deprecated Use processReposPromise instead
 *
 * @private
 * @param {Array} repos
 ...
 * @param options
 */
function processReposPromise(repos, options) {
  if (typeof options !== 'number') {
    options = PROCESS_NIL;
  }
  return new Promise((resolve) => {
    findRepositoriesPromise(options & PROCESS_SKIP_CHECK)
      .then((validRepos) => {
        var all = !repos.length || (repos.length === 1 && repos[0] === '*');
        if (all) {
          // Do all repos
        ...
        if (options & PROCESS_SERIAL) {
          serialPromise(repos, validRepos, all, options)
            .then((res) => {
              resolve(res);
            });
        }
        else {
          parallellPromise(repos, validRepos, all, options)
            .then((res) => {
              resolve(res);
            });
        }
...
function parallellPromise(arr, validRepos, all, options) {
  return new Promise((resolve) => {
    let parallellPromises = [];

    for (var i = 0; i < arr.length; i++) {
      if (isValidRepo(i, arr[i], validRepos, all, options)) {
        let processPromise = processRepoPromise(arr[i]);
        parallellPromises.push(processPromise);
      }
    }

    Promise.all(parallellPromises).then((results) => {
      resolve(results);
    });
  });
}
...
function serialPromise(obj, validRepos, all, options) {
  var id, isArray = obj instanceof Array;
  let reposStatus = [];
  ...
  var next = function () {
    id = isArray ? i : ids[i];
    if (isValidRepo(id, obj[id], validRepos, all, options)) {
      processRepoPromise(arr[i]).then((status) => {
        // We need to use a real async function in order for the stack to clear.
        setTimeout(function () {
          i++;
          reposStatus.push(status);
          if (i === (isArray ? obj.length : ids.length)) {
            resolve();
          }
          else {
            next();
...
let processRepoPromise = function (repo) {
  return new Promise((resolve) => {
 ...
 */
let isValidRepo = function (id, value, validRepos, all, options) {
  if (ignoredRepos.indexOf(value) !== -1 || !(options & PROCESS_SKIP_CHECK) && semiIgnoredRepos.indexOf(value) !== -1) {
    // Ignore repo
...
/**
 * Recursively get all repos and their dependencies as a collection
 *
 * @param repos
 * @return {Promise}
 */
h5p.getDependencies = function (repos) {
  return new Promise((resolve, reject) => {
    let fetchedLibraryDependencies = [];
    repos.forEach(function (repo) {
      const libraryDependencies = getLibraryPromise(repo)
        .catch((err) => {
          reject(err);
        });
      fetchedLibraryDependencies.push(libraryDependencies);
    });

    Promise.all(fetchedLibraryDependencies)
      .then(() => {
        // Libraries and their dependencies
        resolve(Object.keys(collection));
      }, (err) => {
        reject(err);
      });
  });
 ...
 */
h5p.packPromise = function (repos, file) {
  createPackPromise(file, repos);
};

/**
 * Pack the given libraries in a .h5p file
 * @deprecated Use h5p.packPromise instead
 *
 * @param repos
 * @param file
 * @param {function} next print errors or repo info
 */
...
h5p.checkVersions = function () {
  var input = new Input(Array.prototype.slice.call(arguments));
  var inputFile = input.getFileName();
  ...
  processReposPromise(libraries, PROCESS_SKIP_CHECK)
    .then((processedRepos) => {
      console.log("hello!");
    });
};
 ...
 * @deprecated Use createPackPromise instead
 *
 * @private
 * @param {string} file
 * @param {string[]} repos
 ...
 * Makes it easier to create a package based on a list of repos
 *
 * @param {string} file Name of packed h5p file
 * @param {string[]} repos Repos that will be packed
 * @param {function} process
 */
let createPackPromise = function (file, repos) {
  ...
  archive.pipe(output);
  processReposPromise(repos, PROCESS_SKIP_CHECK)
    .then((processedRepos) => {
      processedRepos.forEach(function (procRepo) {
        archiveDir(archive, procRepo.path, procRepo.target);
      });
      archive.finalize();
    });
};
