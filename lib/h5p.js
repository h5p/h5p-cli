...
function parallellPromise(arr, validRepos, all, archive, options) {
  return new Promise(function (resolve) {
    ...
    for (var i = 0; i < arr.length; i++) {
      parallellPromises.push(each(i, arr[i], validRepos, all, archive, options));
    }
...
function serialPromise(obj, validRepos, all, archive, options) {
  ...
  var next = function () {
    id = isArray ? i : ids[i];
    each(id, obj[id], validRepos, all, archive, options).then((status) => {
      // We need to use a real async function in order for the stack to clear.
      setTimeout(function () {
        i++;
...
let processRepoPromise = function (repo, archive) {
  return new Promise(function (resolve) {
...
let each = function (id, value, validRepos, all, archive, options) {
  return new Promise(function (resolve) {
   if (ignoredRepos.indexOf(value) !== -1 || !(options & PROCESS_SKIP_CHECK) && semiIgnoredRepos.indexOf(value) !== -1) {
      // Ignore repo
      resolve({
    ...
    if (all) {
      // No need to check if repo is valid, just process
      processRepoPromise(value, archive)
    ...
    for (var j = 0; j < validRepos.length; j++) {
      if (validRepos[j] === value) {
        processRepoPromise(value, archive).then((res) => {
          resolve(res);
        }, (err) => {
          console.log("FAILED!");
        });
...
h5p.packPromise = function (repos, file) {
  return new Promise(function (resolve, reject) {
    // Package repos, then resolve results of packing repos
    createPackPromise(file, repos).then((reposStatus) => {
      resolve(reposStatus);
    }, (err) => {
      reject(err);
    });
...
let createPackPromise = function (file, repos) {
  return new Promise(function (resolve, reject) {
    ...
    archive.pipe(output);
    processReposPromise(repos, archive, PROCESS_SKIP_CHECK)
      .then(function (res) {
        results = res;
        archive.finalize();
        resolve(res);
