...
/**
 * Recursively add library
 * @param {string[]} directoryNames Directories that should be added to collection
 * @param {Array} directories Current directory data
 */
function recursivelyAddToCollection(directoryName, directories) {
  if (collection[directoryName]) {
    return;
  }

  collection[directoryName] = directoryName;
  // Find processed directory data
  const dir = directories.find((dir) => dir.dirName === directoryName);
  const preloadedDeps = dir.libData.preloadedDependencies || [];
  const editorDeps = dir.libData.editorDependencies || [];
  const dependencies = preloadedDeps.concat(editorDeps);
  ...
  dependencies
    .map(dep => dep.machineName)
    .forEach((dep) => {
      // Find directory from dependency specified as machineName
      const depData = directories.find((dir) => dir.libData.machineName === dep);
      recursivelyAddToCollection(depData.dirName, directories);
    });
}
...
function processRepositories(repos, dirs, options) {
  ...
  if (options & PROCESS_SERIAL) {
    return serialPromise(repos, dirs, all, options);
  }
  else {
    return parallelPromise(repos, dirs, all, options);
  }
}
...
function parallelPromise(repositories, validRepositories, all, options) {
  return Promise.all(
    repositories
      .filter(repo => all || isValidRepo(repo, validRepositories, options))
      .map(repo => processRepoPromise(repo))
  );
}
 ...
 * @return {Promise}
 */
h5p.findDirectories = function (skipCheck) {
  return new Promise((resolve, reject) => {
    fs.readdir('.', function (error, files) {
      if (error) {
...
h5p.pack = function (repos, file) {
  ...
  archive.pipe(output);
  h5p.findDirectories(PROCESS_SKIP_CHECK)
 ...
 *
 * @param {Array} directoryNames Directory name of libraries that will be packed
 * @return {Promise} Resolves with all dependencies of libraries
 */
h5p.getRecursivePackRepos = function (directoryNames) {
  return h5p.findDirectories()
    .then((dirs) => {
      collection = collection || {};
      
      // Get library data for all directories
      const dirData = dirs
        .map((lib) => {
          return {
            dirName: lib,
      ...
      directoryNames.forEach((lib) => {
        recursivelyAddToCollection(lib, dirData);
      });
