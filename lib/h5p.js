 ...
 * @deprecated Use findDirectories instead
 */
 ...
 */
function statusRepository(repo) {
  const status = {
    name: repo
  };
 ...
 */
h5p.status = function (libraries) {
  return findDirectories()
    .then((repositories) => {
      const results = repositories
        .filter(repo =>
          libraries.length > 0 ? libraries.indexOf(repo) >= 0 : true)
...
h5p.pack = function (repos, file) {
  ...
  archive.pipe(output);
  findDirectories(PROCESS_SKIP_CHECK)
    .then((directories) => {
      return processRepositories(repos, directories, PROCESS_SKIP_CHECK)
    })
