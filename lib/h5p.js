...
function checkoutRepository(branch, dir, next) {
  ...
  var proc = child.spawn('git', ['checkout', branch], {cwd: process.cwd() + '/' + dir, env: env});
  ...
  proc.stdout.on('end', function () {
    ...
    var matches = lines[0].match(/^Your branch is (.+)$/);
    if (!matches) {
      status.failed = true;
    }
    else if (matches[1].substr(0, 10) !== 'up-to-date') {
...
h5p.checkout = function (branch, repos, next) {
  findRepositories(function (error, validRepos) {
    ...
    for (var i = 0; i < repos.length; i++) {
      ...
      /**
       * Callback for when checking out is done
       *
       * @private
       * @param {Object} status
       */
      ...
      if (all) {
        checkoutRepository(branch, repos[i], done);
      }
      else {
        // Check to see if valid repo
        var found = false;
