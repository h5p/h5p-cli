...
function getLibraryPromise(libraryName) {
  return new Promise(function (resolve, reject) {
    ...
    getRegistryPromise().then(function (libraries) {
 ...
 * @param options
 */
function processReposPromise(repos, archive, options) {
  if (typeof options !== 'number') {
    options = PROCESS_NIL;
  }
  return new Promise(function (resolve) {
    findRepositoriesPromise(options & PROCESS_SKIP_CHECK)
      .then((validRepos) => {
        ...
        if (options & PROCESS_SERIAL) {
          serialPromise(repos, validRepos, all, archive, options)
            .then((res) => {
              resolve(res);
            });
        }
        else {
          parallellPromise(repos, validRepos, all, archive, options)
            .then((res) => {
              resolve(res);
            });
        }
...
function parallellPromise(arr, validRepos, all, archive, options) {
  return new Promise(function (resolve) {
    let parallellPromises = [];

    for (var i = 0; i < arr.length; i++) {
      parallellPromises.push(validateRepo(i, arr[i], validRepos, all, archive, options));
    }
...
function serialPromise(obj, validRepos, all, archive, options) {
  var id, isArray = obj instanceof Array;
  let reposStatus = [];
  ...
  var next = function () {
    id = isArray ? i : ids[i];
    validateRepo(id, obj[id], validRepos, all, archive, options).then((status) => {
      // We need to use a real async function in order for the stack to clear.
      setTimeout(function () {
        i++;
...
let validateRepo = function (id, value, validRepos, all, options) {
  return new Promise(function (resolve) {
    if (ignoredRepos.indexOf(value) !== -1 || !(options & PROCESS_SKIP_CHECK) && semiIgnoredRepos.indexOf(value) !== -1) {
      // Ignore repo
    ...
    for (var j = 0; j < validRepos.length; j++) {
      if (validRepos[j] === value) {
        processRepoPromise(value).then((res) => {
          resolve(res);
        });
        return;
      }
